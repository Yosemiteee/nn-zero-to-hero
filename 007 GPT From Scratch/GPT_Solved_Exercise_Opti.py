...
# tmux list-sessions
# tmux kill-session -t gpt_opti
#
# 52.934737M parameters
...
class FeedFoward(nn.Module):
    """Replaced the simple linear layer followed by a non-linearity with a Gated Linear Unit (GLU)"""
    def __init__(self, n_embd):
        super().__init__()
        self.gate = nn.Linear(n_embd, 4 * n_embd)      # Gate projection
        self.up_proj = nn.Linear(n_embd, 4 * n_embd)   # Value projection
        self.down_proj = nn.Linear(4 * n_embd, n_embd) # Output projection
        self.dropout = nn.Dropout(dropout) # Dropout

    def forward(self, x):
        gate = self.gate(x)        # Gate projection
        up = self.up_proj(x)       # Value projection
        gated = F.gelu(gate) * up  # Element-wise multiplication after GELU to form the gated output (GLU)
        return self.dropout(self.down_proj(gated)) # Output projection followed by dropout
...
torch.save(model, "gpt_opti_shakespeare.pt")
